...
bfv_matrix Fixed_Attention::forward(const bfv_matrix &input) const {
    ...
    if (party->party == sci::ALICE) {
        ...
        FixArray exp_sum(sci::ALICE, batch_size * batch_size, true, DEFAULT_ELL, DEFAULT_SCALE);
        int mask = fix_rs2_exp_score.ell_mask();
        for (size_t i = 0; i < batch_size; i++) {
            for (size_t j = 0; j < batch_size; j++) {
                exp_sum.data[i * batch_size] += fix_rs2_exp_score.data[i * batch_size + j] & mask;
            }
            for (size_t j = 1; j < batch_size; j++) {
                exp_sum.data[i * batch_size + j] = exp_sum.data[i * batch_size];
            }
        }
        ...
        bfv_matrix Db(batch_size), Rb(batch_size * d_k);
        ...
        bfv_matrix output(batch_size * d_k);
        for (size_t i = 0; i < batch_size; i++) {
            for (size_t j = 0; j < d_k; j++) {
                output[i * d_k + j] = rb2 / (Db[i] * Rb[j]); // should not be like this
            }
        } 
