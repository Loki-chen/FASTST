...
BFVLongCiphertext FixedLayerNorm::forward(const BFVLongCiphertext &attn, const bfv_matrix &input) const
{
    ...
    if (party->party == ALICE)
    {
        ...
        FixArray fix_input = fixop->input(party->party, input.size(), input.data(), true, DEFAULT_BITWIDTH, DEFAULT_ELL);
        ...
        */
        ...
        FixArray fix_ha2xgb = fixop->input(sci::PUBLIC, ha2xgb.size(), ha2xgb.data(), true, DEFAULT_BITWIDTH, DEFAULT_ELL);
        ...
        FixArray fix_tmp1 = fix_public->input(sci::PUBLIC, tmp1.size(), tmp1.data(), DEFAULT_BITWIDTH, DEFAULT_ELL);
