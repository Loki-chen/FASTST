...
uint64_t *Conversion::Ring_to_Prime(uint64_t *input, int length, int ell,
                                    int64_t plain_mod) {
#ifdef LOG
...
// P-to-R
uint64_t *Conversion::Prime_to_Ring(int party, uint64_t *input, int length,
                                    int ell, u_int64_t plain_prime, int s_in,
                                    int s_out, FPMath *fpmath) {
    // if input > plain_prime, then sub plain_prime
    // sub plain_prime/2 anyway
    uint64_t *output = new uint64_t[length];
    FixArray fix_input =
        fpmath->fix->input(party, length, input, true, ell, s_in);
    FixArray p_array =
        fpmath->fix->input(sci::PUBLIC, length, plain_prime, true, ell, s_in);
    FixArray p_2_array = fpmath->fix->input(
        sci::PUBLIC, length, (plain_prime - 1) / 2, true, ell, s_in);
